...
def load_portfolio(portfolio_name: str = "default") -> list:
    """Główna funkcja skanująca wszystkie pliki i budująca 1 zbiorczy raport."""
    os.chdir(BASE_DIR)

    all_active_positions = []
    all_tickers_to_scan = set()

    # Wczytywanie wszystkich portfeli z katalogu portfolios/
    if PORTFOLIOS_DIR.exists():
        for file_path in PORTFOLIOS_DIR.glob("*.json"):
          try:
            ...
            if isinstance(data, dict) and "positions" in data:
              for pos in data["positions"]:
                all_active_positions.append(pos)
                all_tickers_to_scan.add(pos["symbol"])
              ...
              all_tickers_to_scan.update(data)
    
          except Exception as e:
            print(f"❌ Błąd wczytywania {file_path.name}: {e}")
    
        if not all_tickers_to_scan:
            print("⚠️ Brak tickerów do skanowania. Używam domyślnych.")
            all_tickers_to_scan = {"NVDA", "AAPL", "MSFT", "KRU.WA", "PKO.WA"}
            return []
        
        print(
            f"🚀 ZBIORCZA ANALIZA DLA {len(all_tickers_to_scan)} SPÓŁEK ORAZ"
            f" {len(all_active_positions)} POZYCJI AKTYWNYCH..."
        )
        return all_tickers_to_scan
